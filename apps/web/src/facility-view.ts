...
import {
  ...
  feeOf, headroom, holderClaim, indicativeMark, issuedLtvBp, purchase, reconcile, retainedMinimum,
...
/** Indicative marks before settlement. Frozen settlement values override these. */
export function markView(collateralIdr: bigint, recoveryBp: bigint, costsIdr: bigint, bands: Band[]) {
  const senior = big(bands.find((band) => band.name === "SENIOR")?.subscribedIdr);
  const junior = big(bands.find((band) => band.name === "JUNIOR")?.subscribedIdr);
  return { ...indicativeMark(collateralIdr, recoveryBp, costsIdr, senior, junior), seniorFaceIdr: senior, juniorFaceIdr: junior };
}
...
/**
 * The API still answers `purchasePriceIdr: 0` for every band, so the screens
 * would otherwise show face value where the discounted price belongs. Price
 * the book here from the band's own yield and the facility term, and say so.
 */
export function bandPricing(band: Band, termDays: bigint) {
  const serverPrice = big(band.purchasePriceIdr);
  const capacityIdr = big(band.capacityIdr);
  const subscribedIdr = big(band.subscribedIdr);
  const yieldBp = big(band.returnBp);
  return {
    capacityIdr,
    subscribedIdr,
    yieldBp,
    capacityPriceIdr: purchase(capacityIdr, yieldBp, termDays),
    subscribedPriceIdr: subscribedIdr > 0n ? purchase(subscribedIdr, yieldBp, termDays) : 0n,
    provenance: (serverPrice > 0n ? "server" : "derived") as Provenance,
  };
}
