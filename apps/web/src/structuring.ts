...
/** Stable across reloads, so a proposal can be told apart from a re-preview. */
export function proposalDigest(input: StructuringInput, structure: Structure | null): string {
  const parts = [
    input.policy.policyHash,
    input.policy.modelVersion,
    input.report.reportHash,
    String(input.requestedFaceIdr),
    String(input.authorizedFaceCeilingIdr),
    String(input.termDays),
    String(input.upfrontCostsIdr),
    structure ? `${structure.seniorCapIdr}:${structure.juniorCapIdr}` : "infeasible",
  ].join("|");
  let hash = 0x811c9dc5;
